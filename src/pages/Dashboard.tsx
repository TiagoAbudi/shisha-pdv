...
import { ShoppingCart, Package, FileText, Settings, Users, LogOut, BarChart3 } from 'lucide-react';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: cores.bgGeral, color: cores.texto }}>
      ...
      <header style={{ backgroundColor: cores.header, color: cores.texto, padding: '8px 20px', display: 'flex', gap: '24px', fontSize: '13px', fontWeight: '500', borderBottom: `1px solid ${cores.borda}` }}>
        <div style={{ cursor: 'pointer', opacity: 0.8 }} onClick={() => alert('Menu Arquivo (Em desenvolvimento)')}>Arquivo</div>
        ...
        <div style={{ cursor: 'pointer', opacity: 0.8 }} onClick={() => alert('Sistema de PDV Shisha Conveniência v1.0\n\nAtalhos:\n[F2] Caixa\n[F3] Produtos\n[F4] Notas\n[F7] Clientes\n[F9] Relatórios\n[F10] Configurações')}>Ajuda</div>
      </header>
      ...
      <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '24px', maxWidth: '1000px', width: '100%' }}>
          ...
          {[
            ...
            <button
              key={i} onClick={() => mudarTela(btn.action as Telas)}
